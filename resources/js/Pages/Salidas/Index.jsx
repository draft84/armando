...
import EditSalidaModal from '@/Components/EditSalidaModal';
...
export default function SalidasIndex({ salidas, totales, filters }) {
    ...
    const [salidaToEdit, setSalidaToEdit] = useState(null);

    // Búsqueda en tiempo real con debounce
    const handleSearch = (value) => {
        setSearch(value);
        const timeoutId = setTimeout(() => {
            router.get('/salidas', { search: value, perPage }, {
                preserveState: true,
                preserveScroll: true,
            });
        }, 300);
    ...
    const handlePerPageChange = (newPerPage) => {
        ...
        router.get('/salidas', { search, perPage: newPerPage }, {
            preserveState: true,
            preserveScroll: true,
        });
    };
    ...
    useEffect(() => {
        if (salidaToEdit) {
            editForm.setData({
                id: salidaToEdit.id,
                items: salidaToEdit.items || '',
                fecha: salidaToEdit.fecha ? new Date(salidaToEdit.fecha).toISOString().slice(0, 16) : '',
                lote: salidaToEdit.lote || '',
                codigo: salidaToEdit.codigo || '',
                caja: salidaToEdit.caja || '',
                especie: salidaToEdit.especie || '',
                producto: salidaToEdit.producto || '',
                calidad: salidaToEdit.calidad || 'A',
                fecha_elab: salidaToEdit.fecha_elab ? new Date(salidaToEdit.fecha_elab).toISOString().slice(0, 16) : '',
                fechavenci: salidaToEdit.fechavenci ? new Date(salidaToEdit.fechavenci).toISOString().slice(0, 16) : '',
                talla: salidaToEdit.talla || '',
                uds: salidaToEdit.uds || '',
                libras: salidaToEdit.libras || '',
                quees: salidaToEdit.quees || '',
                empaque: salidaToEdit.empaque || '',
                cuarto: salidaToEdit.cuarto || '',
                posicion: salidaToEdit.posicion || '',
                tarima: salidaToEdit.tarima || '',
                cliente: salidaToEdit.cliente || '',
                ndoc: salidaToEdit.ndoc || '',
            });
        }
    }, [salidaToEdit]);
    ...
    const handleEditSubmit = (e) => {
        e.preventDefault();
        router.put(`/salidas/${editForm.data.id}`, {
            items: editForm.data.items,
            fecha: editForm.data.fecha,
            ...
            cliente: editForm.data.cliente,
            ndoc: editForm.data.ndoc,
        }, {
            preserveScroll: true,
            onSuccess: () => {
                closeEditModal();
    ...
    const handleDeleteConfirm = () => {
        if (salidaToDelete) {
            router.delete(route('salidas.destroy', salidaToDelete.id), {
                onSuccess: () => {
                    closeDeleteModal();
    ...
    const handleImportSubmit = async (file) => {
        ...
        try {
            ...
            const response = await fetch('/salidas/import', {
                method: 'POST',
                body: formData,
                headers: headers,
            ...
            if (contentType && contentType.includes('application/json')) {
                ...
                if (response.ok && data.success) {
                    ...
                    error(data.message || 'Error desconocido al importar');
                }
            } else {
                if (response.ok) {
                    ...
                    error('Error al procesar la solicitud (HTTP ' + response.status + ')');
                }
            }
        } catch (error) {
    ...
    const fechaMasCercana = useMemo(() => {
        if (!salidas.data || salidas.data.length === 0) return null;
        const hoy = new Date();
        let fechaCercana = null;
        let menorDiferencia = Infinity;

        salidas.data.forEach(salida => {
            const fechaEmp = new Date(salida.fecha);
            const diferencia = Math.abs(fechaEmp - hoy);
            if (diferencia < menorDiferencia) {
                menorDiferencia = diferencia;
                fechaCercana = salida.fecha;
            }
        });
            ...
            <div className="card mb-6">
                <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                    ...
                    <div className="relative flex-1 max-w-md">
                        ...
                        <input
                            type="text"
                            placeholder="Buscar por lote, código, especie, cliente..."
                            value={search}
                    ...
                    <div className="flex items-center space-x-2">
                        ...
                        <button
                            ...
                            className="px-4 py-2 bg-red-600 hover:bg-red-700 text-white rounded-lg transition-all font-medium text-sm shadow-sm flex items-center"
                        >
                            ...
                            {salidas.data && salidas.data.length > 0 ? (
                                salidas.data.map((salida) => (
                                    ...
                                    >
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-gray-900 dark:text-white">
                                            {new Date(salida.fecha_elab).toLocaleDateString('es-ES')}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-gray-900 dark:text-white">
                                            {new Date(salida.fechavenci).toLocaleDateString('es-ES')}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap font-medium text-red-600 dark:text-red-400">
                                            {salida.caja2.toLocaleString()}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-gray-900 dark:text-white">
                                            {salida.talla}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-gray-900 dark:text-white">
                                            {salida.libras ? Number(salida.libras).toFixed(2) : '-'}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap font-medium text-red-600 dark:text-red-400">
                                            {salida.promedio ? Number(salida.promedio).toFixed(2) : '-'}
                                        </td>
                                        <td className="px-4 py-3 whitespace-nowrap text-gray-900 dark:text-white">
                                            {salida.quees || '-'}
                                        </td>
                                        ...
                                        <td className="px-4 py-3 whitespace-nowrap text-right">
                                            <div className="flex items-center justify-end space-x-1">
                                                ...
                                                <button
                                                    onClick={() => openDeleteModal(salida)}
                                ...
                                <tr>
                                    <td colSpan="23" className="px-4 py-8 text-center text-gray-500 dark:text-gray-400">
                    ...
                    <div className="px-4 py-3 border-t border-gray-200 dark:border-gray-700">
                        <div className="flex flex-col md:flex-row items-center justify-between gap-4">
                            <div className="text-sm text-gray-600 dark:text-gray-400">
                                Mostrando {salidas.from} a {salidas.to} de {salidas.total} resultados
                            </div>
                            <div className="flex space-x-2">
                                {salidas.links.map((link, index) => (
                                    link.url ? (
                                        <Link
                                            key={index}
                                            href={link.url}
